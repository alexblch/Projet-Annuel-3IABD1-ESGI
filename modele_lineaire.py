...
if __name__ == "__main__":
    ...
    def test_linear_model(img_list, class_size, img_size, path_file_model_football, path_file_model_basket, path_file_model_tennis, my_lib):
        func = my_lib.test_linear_model
        ctype_img_list = (ctypes.POINTER(ctypes.c_int) * len(img_list))()
        for i in range(len(img_list)):
            sublist = img_list[i]
        ...
        func.restype = ctypes.POINTER(ctypes.POINTER(ctypes.c_int))
        list_train = func(ctype_img_list, ctype_class_size, img_size, path_file_model_football.encode(), path_file_model_basket.encode(), path_file_model_tennis.encode())
        # list_train = [[4, 5, 6], [7, 8, 9]]
        return list_train
    ...
    #IMPORTANT Mettre les chemins absolu des fichiers
    path_file_model_football = "football_model.txt"
    path_file_model_basket = "basket_model.txt"
    path_file_model_tennis = "tennis_model.txt"
    ...
    class_size_test = np.array([len(foot_list_test), len(basket_list_test), len(tennis_list_test)])
    ctype_class_size_test = class_size_test.ctypes.data_as(ctypes.POINTER(ctypes.c_int))
    label_and_output = test_linear_model(img_list_test, class_size_test, largeur_img * largeur_img, path_file_model_football, path_file_model_basket, path_file_model_tennis,
                                         my_lib)
    img_list_size_test = len(foot_list_test) + len(basket_list_test) + len(tennis_list_test)
    success = 0
    test_result = []
